...
describe('passageFromJSON()', () => {
	...
	it('converts a passage with empty tags properly', () => {
		expect(passageFromJSON(':: No Tags []')).toEqual(
			passageObject({
				name: 'No Tags',
				tags: [],
				text: ''
	...
	it('converts a passage with empty metadata properly', () => {
		expect(passageFromJSON(':: No Metadata {}')).toEqual(
			passageObject({
				name: 'No Metadata',
				tags: [],
				text: ''
		...
		expect(passageFromJSON(':: No Metadata Or Tags [] {}')).toEqual(
			passageObject({
	...
	it('ignores malformed metadata', () => {
		...
		expect(passageFromJSON(':: Bad Metadata {1 + 1}')).toEqual(
			passageObject({
				name: 'Bad Metadata',
				tags: [],
				text: ''
		...
		expect(passageFromJSON(":: This Isn't Metadata {1 + 1")).toEqual(
			passageObject({
		...
		expect(
			passageFromJSON(':: Bad Position {"position": 32,"size":"10,20"}')
		).toEqual(
			passageObject({
				height: 20,
				name: 'Bad Position',
		...
		expect(
			passageFromJSON(':: Bad Size {"position":"10,20","size": 32}')
		).toEqual(
			passageObject({
				left: 10,
				name: 'Bad Size',
	...
	it('throws an error if given a passage with no name or text', () =>
		expect(() => passageFromJSON('::')).toThrow());

	it('converts a passage with no text properly', () =>
		expect(passageFromJSON(':: No Text')).toEqual(
			passageObject({
				name: 'No Text',
				tags: [],
				text: ''
...
describe('passageToJSON()', () => {
	it('converts a passage with no tags properly', () => {
		const passage = fakePassage({name: 'mock-passage', tags: []});

		expect(passageToJSON(passage)).toBe(
...
describe('storyFromJSON()', () => {
	...
	it('applies metadata in a StoryData passage', () => {
		...
		expect(storyFromJSON(`:: StoryData\n${JSON.stringify(data)}`)).toEqual(
			expect.objectContaining({
				ifid: story.ifid,
	...
	it('arranges passages in a grid 10 passages wide if none have any position metadata set', () => {
		...
		const {passages} = storyFromJSON(source);
...
describe('storyToJSON()', () => {
	...
	it('creates a StoryTitle passage', () =>
		expect(storyToJSON(story)).toContain(`:: StoryTitle\n${story.name}\n\n`));
	...
	it('creates a StoryData passage with formatted JSON', () => {
		...
		expect(storyToJSON(story)).toContain(
			`:: StoryData\n{\n  "ifid": "${story.ifid}",\n  "format": "${story.storyFormat}",\n  "format-version": "${story.storyFormatVersion}",\n  "start": "${story.passages[0].name}",\n  "tag-colors": {\n    "tag-name": "red"\n  },\n  "zoom": ${story.zoom}\n}\n`
		);
	});
	...
	it('creates a passage tagged "script" if the story has a script property', () => {
		...
		expect(storyToJSON(story)).toMatch(
			`:: StoryScript [script]\n${story.script}`
		);
	});
