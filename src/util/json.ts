...
/**
 * Escapes characters with special meanings in a JSON passage header (brackets,
 * curly quotes, and backslashes).
 */
export function escapeForJSONHeader(value: string) {
 ...
 * the start of a line.
 */
export function escapeForJSONText(value: string) {
	return value.replace(/^::/gm, '\\::');
}

/**
 * Converts a single passage to JSON.
 */
export function passageToJSON(passage: Passage) {
	const escapedName = escapeForJSONHeader(passage.name)
		.replace(/^\s+/g, match => '\\ '.repeat(match.length))
		.replace(/\s+$/g, match => '\\ '.repeat(match.length));
	const tags =
		passage.tags.length > 0
			? `[${passage.tags.map(escapeForJSONHeader).join(' ')}]`
			: undefined;
	const metadata = JSON.stringify({
		position: `${passage.left},${passage.top}`,
		size: `${passage.width},${passage.height}`
	}).replace(/\s+/g, '');
	const escapedText = escapeForJSONText(passage.text);
...
/**
 * Converts JSON source to a passage. If it can't be parsed, then an error is
 * thrown. If it can partially parse the passage, it will do so.
 */
export function passageFromJSON(source: string): Omit<Passage, 'story'> {
	...
	const passage: Omit<Passage, 'story'> = {
		...passageDefaults(),
		id: uuid(),
		name: unescapeForJSONHeader(
			rawName
				.replace(/^(\\\s)+/g, match => ' '.repeat(match.length / 2))
				.replace(/(\\\s)+$/g, match => ' '.repeat(match.length / 2))
		),
		tags: [],
		text: lines.map(unescapeForJSONText).join('\n').trim()
	};
	...
	if (rawTags) {
		...
		passage.tags = rawTags
			...
			.split(/\s/)
			.filter(tag => tag.trim() !== '')
			.map(tag => unescapeForJSONHeader(tag));
	}
...
/**
 * Converts a story from JSON source.
 */
export function storyFromJSON(source: string) {
	const id = uuid();
	...
	if (dataPassageIndex !== -1) {
		...
		console.warn('No StoryData passage is present in JSON');
	}

	// Detect old JSON format, which would have no passage metadata, and put
	// passages in a grid.
...
/**
 * Converts a story to JSON.
 */
export function storyToJSON(story: Story) {
	const storyTitle = `:: StoryTitle\n${escapeForJSONText(story.name)}`;
	...
	// passages. These passage names are not part of the JSON spec.
...
/**
 * Unescapes characters with special meanings in a JSON passage header (brackets,
 * curly quotes, and backslashes).
 */
export function unescapeForJSONHeader(value: string) {
 ...
 * the start of a line.
 */
export function unescapeForJSONText(value: string) {
	return value.replace(/^\\:/gm, ':');
}
